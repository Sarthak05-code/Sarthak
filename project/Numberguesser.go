...
func gameLogic(difficulty int) {
	...
	// Single persistent reader goroutine for the whole game.
	// This avoids spawning a new goroutine per turn, which left
	// orphaned readers racing on stdin after a timeout.
	go func() {
		for {
			var input int
